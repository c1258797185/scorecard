...
def missing_data_Y(missing_data,data,label):
"""
得出每个缺失字段中，缺失值下坏样本比例和非缺失值下坏样本比例，目的是判断缺失值比例高的字段是否删除或者做特征衍生
missing_data:每个字段缺失值比例中筛选缺失值大于0的
data:原数据
label:Y标签
"""
    missing_data['bad_rate_null']=''
    missing_data['bad_rate_notnull']=''
    for i in range(len(missing_data)):
        col=missing_data['feature_name'][i]
        df1_0=data[data[col].isnull()]
        df_0_bad_rate=(df1_0[df1_0[label]==1].shape[0])/df1_0.shape[0]
        df1_1=data[data[col].notnull()]
        df_1_bad_rate=(df1_1[df1_1[label]==1].shape[0])/df1_1.shape[0]
        missing_data['bad_rate_null'][i]=df_0_bad_rate
        missing_data['bad_rate_notnull'][i]= df_1_bad_rate
return missing_data
...
#计算0值比例
def draw_0_data_table(df):
    total=(df== 0).sum(axis=0).sort_values(ascending=False)
    percent =((df== 0).sum(axis=0) /df.shape[0]).sort_values(ascending=False)
    data0=pd.concat([total, percent], axis=1, keys=["Total", "Percent"])
    data0.reset_index(inplace=True)
    data0.rename(columns={"index": "feature_name"}, inplace=True)
    return data0


# for i in col_na_30:
#     df[i]=df[i].apply(lambda x: 1 if math.isnan(x) else 0)